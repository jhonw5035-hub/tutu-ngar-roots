...
/* ------------------------------------------------------------------ */
/* Derived view-model helpers for the card-based passenger screens.     */
/* These are computed from the arrays above — no separate data source.  */
/* ------------------------------------------------------------------ */

export type TimeBand = "any" | "morning" | "afternoon" | "evening";

export const timeBandLabel: Record<TimeBand, string> = {
  any: "Any time",
  morning: "Morning",
  afternoon: "Afternoon",
  evening: "Evening",
...
/** Straight-line distance between two pickup points, in km. */
export function distanceBetweenPoints(a: PickupPoint, b: PickupPoint) {
...
/** Time slots for a route, enriched with arrival, seats left and a pick. */
export function getSlotDetails(routeId: string | null): SlotDetail[] {
  ...
  const details = getSlotsForRoute(routeId).map((slot) => {
    ...
    return {
      ...
      price: route.fare,
      womenOnlyAvailable: riders.every((r) => r.gender === "female"),
      recommended: false,
    };
  });

  // Recommend the soonest departure that still has comfortable availability.
  const pick =
    details.find((d) => d.seatsLeft >= 2) ?? details.find((d) => d.seatsLeft >= 1);
...
/** Two hand-picked corridors shown as quick-select cards on Home. */
...
export const upcomingTrip: UpcomingTrip = {
  id: "t-2001",
  routeId: "r-kabaraye",
  routeName: "Kabar Aye Pagoda Road Corridor",
  pickup: "Kabar Aye Pagoda",
  destination: "Botahtaung",
  date: "21 Aug 2026",
  time: "08:15",
  fare: 2800,
  status: "Confirmed",
