...
import { replyToSupportMessage, resolveSupportMessage, useAllSupportMessages } from "@/lib/support";
import {
...
function SupportPage() {
  ...
  const repeatKeys = React.useMemo(() => {
    ...
    for (const c of rows) {
      const subject =
        c.target_type === "vehicle"
          ? (c.driver_id ?? "unknown-vehicle")
          : (c.driver_id ?? "unknown-driver");
  ...
  const filtered = rows.filter(
    (c) =>
      ...
      (target === "all" || c.target_type === target),
  );
              ...
              return (
                <article key={c.id} className="rounded-xl border border-border bg-card p-4">
                  ...
                  <p className="mt-2 text-xs text-muted-foreground">
                    ...
                    {subject
                      ? ` · about ${displayName(subject)}${subject.plate_number ? ` (${subject.plate_number})` : ""}`
                      : ""}
                  </p>
  ...
  if (messages.length === 0) {
    return (
      <div className="mt-4 rounded-xl border border-dashed border-border bg-card p-10 text-center text-sm text-muted-foreground">
        No direct support messages yet. Anything a passenger sends from “Contact Support” lands here
        instantly.
      </div>
    );
  }
              ...
              <div className="flex gap-2">
                ...
                {m.status !== "resolved" ? (
                  <Button
                    size="sm"
                    variant="outline"
                    onClick={() => void resolveSupportMessage(m.id)}
                  >
                    Mark resolved
                  </Button>
                ) : null}
